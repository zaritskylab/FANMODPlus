...
static boolean_nauty filterschreier(schreier*,int*,permnode**,boolean_nauty,int,int);
...
static boolean_nauty
filterschreier(schreier *gp, int *p, permnode **ring,
               boolean_nauty ingroup, int maxlevel, int n)
 ...
 * Return TRUE iff some change is made. */
{
    ...
    int *orbits,*pwr;
    permnode **vec,*curr;
    boolean_nauty changed,lchanged,ident;
#if !MAXN
...
getorbitsmin(int *fix, int nfix, schreier *gp, permnode **ring,
         int **orbits, int *cell, int ncell, int n, boolean_nauty changed)
